...
function EditWeeklyMenu() {
    ...
    const fetchMenu = async () => {
      try {
        const result = await axios.get(
          `${serverUrl}/api/weekly/menu/by-id/${menuId}`,
        ...
        setFormData({
          ...
          pricePerWeek: menu.pricePerWeek,
          category: menu.category,
          weeklySchedule: menu.weeklySchedule,
        });
        setFrontendImage(menu.image);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${serverUrl}/api/weekly/menu/edit/${menuId}`,
        submitData,
        { withCredentials: true }
      );
      setLoading(false);
  ...
  if (fetchingMenu) {
    return (
      <div className="min-h-screen bg-gray-50 pt-[90px] pb-10 flex items-center justify-center">
        <Nav />
        <ClipLoader color="#D40000" size={50} />
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-10">
          <h1 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
            <FaCalendarAlt className="text-[#D40000]" /> Edit Weekly Plan
          </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  required
                  type="text"
                  placeholder="e.g. Corporate Veg Lunch"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-[#D40000] text-white rounded-xl font-bold hover:bg-[#b00000] transition disabled:opacity-50"
            >
              {loading ? "Updating..." : "Update Weekly Menu"}
            </button>
          </form>
        </div>
      </div>
