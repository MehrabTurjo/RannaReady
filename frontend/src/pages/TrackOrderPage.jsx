...
//          <div>
//             <p className='text-lg font-bold mb-2 text-[#D40000]'>{shopOrder.shop.name}</p>
...
function TrackOrderPage() {
    ...
    // --- FIX 1: Socket Listener with Cleanup ---
    // This handles the real-time updates without needing a reload
    useEffect(() => {
        if (!socket) return; // Guard clause in case socket is null

        const handleLocationUpdate = ({ deliveryBoyId, latitude, longitude }) => {
            console.log("Socket Update Received:", latitude, longitude); // Debugging log
            setLiveLocations(prev => ({
                ...prev,
                [deliveryBoyId]: { lat: latitude, lon: longitude }
            }));
    ...
    // --- FIX 2: Move handleGetOrder INSIDE useEffect ---
    // This stops the infinite re-render loop error
    useEffect(() => {
        const handleGetOrder = async () => {
            try {
    ...
    }, [orderId]); // Only re-runs if the Order ID changes
